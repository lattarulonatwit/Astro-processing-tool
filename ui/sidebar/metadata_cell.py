...
class MetadataCell(tk.Frame):
     def __init__(self, parent, starsToDetect, on_starID,):
        ...
        self.ISOLabel = tk.Label(self.captureTab, text="Camera ISO Speed: N/A")
        self.ISOLabel.grid(row=3, column=0, sticky="ew", pady=(5, 0))

        #star data
     #   self.pixelScale = tk.Label(self.astroTab, text = "Image Pixel Scale: N/A")
    #    self.pixelScale.grid(row = 1, column = 0, sticky = "ew", pady = (5, 0))

        self.numStarsLabel = tk.Label(self.astroTab, text = "Number of stars found: N/A")
        self.numStarsLabel.grid(row = 2, column = 0, sticky = "ew", pady = (5, 0))

        self.brightestLabel = tk.Label(self.astroTab, text="Brightest Star Magnitude: N/A")
        self.brightestLabel.grid(row=3, column=0, sticky="ew", pady=(5, 0))

     def updateLabels(self, focalLength, shutterSpeed, ISO, numStars, brightest):
        #Mess with the labels
        self.focalLengthLabel["text"] = f"Telescope Focal Length: {focalLength}"
        self.shutterSpeedLabel["text"]= f"Camera Exposure Time: {shutterSpeed}"
        self.ISOLabel["text"] = f"Camera ISO Speed: {ISO}"

      #  self.pixelScale["text"] = f"Image Pixel Scale: {focalLength}"
